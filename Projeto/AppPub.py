        ...
        if event in (sg.WINDOW_CLOSED, "-FINALIZAR-"):
            ...
            if not parametro:
                ...
                if parametro == "Título":
                    ...
                    novas_afiliacoes = sg.popup_get_text("Insira as afiliações no formato '[autor índice]:afiliacao'", font=("Helvetica", 15))
                    if novas_afiliacoes:
                        afiliacoes_dict = {}
                        for afil in novas_afiliacoes.split(","):
                            try:
                                autor_idx, afiliacao = afil.split(":")
                                afiliacoes_dict[int(autor_idx.strip())] = afiliacao.strip()
                            except ValueError:
                                sg.popup_error("Erro no formato! Use '[autor índice]:afiliacao'", font=("Helvetica", 15))
                        

                        for autor_idx, afiliacao in afiliacoes_dict.items():
                            if 0 <= autor_idx < len(selected_pub["authors"]):
                                selected_pub["authors"][autor_idx]["affiliation"] = afiliacao
                            else:
                                sg.popup_error(f"O índice {autor_idx} não é válido.", font=("Helvetica", 15))
...
def listar_autores_e_publicacoes(dataset):
    ...
    if not autores_ordenados:  # Se não houver ordenação, sai da função
        return
    ...
    if autores_ordenados:
        ...
        res_layout = [
            [
                sg.Table(
                    values=res,
                    headings=["Autor", "Nº Publicações", "Publicações"],
                    auto_size_columns=False,
                    col_widths=col_widths,
                    justification="center",
                    key="-TABELA-",
        ...
        res_window = sg.Window(
            "Análise de Autores e Publicações",
            res_layout,
            font=("Helvetica", 15),
            resizable=True,
            finalize=True,
        ...
        while not stop1:
            ...
            # Verificar se o usuário clicou em uma linha
            elif event == "-TABELA-" and values["-TABELA-"]:
                selected_row_index = values["-TABELA-"][0]  # Obtem o índice da linha selecionada
                if selected_row_index is not None:
                    selected_row = res[selected_row_index]
                    autor = selected_row[0]
                    publicacoes = selected_row[2]

                    # Mostrar publicações completas em uma nova janela
                    pub_layout = [
                        [sg.Text(f"Autor: {autor}", font=("Helvetica", 15))],
                        [sg.Multiline(publicacoes, size=(80, 20), font=("Helvetica", 12), key="-PUBLICACOES-", disabled=True)],
                        [sg.Button("Fechar", key="-CLOSE-PUBS-")]
                    ]

                    pub_window = sg.Window("Publicações", pub_layout, font=("Helvetica", 15), resizable=True)
                    pub_stop = False
                    while not pub_stop:
                        pub_event, pub_values = pub_window.read()
                        if pub_event in (sg.WINDOW_CLOSED, "-CLOSE-PUBS-"):
                            pub_stop = True
                    pub_window.close()
...
def analise_por_palavras_chave(dataset):
    ...
    while not stop:
        ...
        if event == sg.WINDOW_CLOSED or event == "-CANCELAR-":
            ...
            stop = True
            ord_window.close()
    if not palavras_chave_ordenadas:  
        return
    ...
    res_layout = [
        [
            sg.Table(
                values=res,
                headings=["Palavra-chave", "Ocorrências", "Publicações"],
                col_widths=col_widths,
                auto_size_columns=False,
                justification="center",
                key="-TABELA-",
                enable_events=True,
                vertical_scroll_only=False,  # Permite scroll horizontal
                num_rows=15,  # Número de linhas visíveis
                expand_x=True,
                expand_y=True,
            )
        ],
    ...
    res_window = sg.Window(
        "Análise de Palavras-chave e Publicações",
        res_layout,
        font=("Helvetica", 15),
    ...
    while not stop1:
        ...
        # Verificar se o usuário clicou em uma linha
            ...
            if selected_row_index is not None:
                selected_row = res[selected_row_index]
                palavra_chave = selected_row[0]
                ocorrencias = selected_row[1]
                publicacoes = selected_row[2]

                # Mostrar publicações completas em uma nova janela
                pub_layout = [
                    [sg.Text(f"Palavra-chave: {palavra_chave}", font=("Helvetica", 15))],
                    [sg.Text(f"Ocorrências: {ocorrencias}", font=("Helvetica", 15))],
                    [sg.Multiline(publicacoes, size=(80, 20), font=("Helvetica", 12), key="-PUBLICACOES-", disabled=True)],
                    [sg.Button("Fechar", key="-CLOSE-PUBS-")]
                ]
    ...
    res_window.close()
    res_pesquisa["criterios"] = "Pesquisa por palavras-chave e publicações associadas"
    res_pesquisa["resultados"] = res
